...
// const PERFUMES = [
//   { id: 1, name: "Boutique Workspace", category: "RETAIL", desc: "A workspace that doubles as a showroom. Curated material samples line the walls like a gallery—every surface a demonstration of the studio's craft.", src: 'https://images.unsplash.com/photo-1547887537-6158d64c35b3?w=500&auto=format&fit=crop&q=60' },
//   { id: 2, name: "Modern Conference", category: "COMMERCIAL", desc: "A boardroom built for decisions that matter. Integrated AV technology disappears into the architecture—only the clean lines remain.", src: 'https://images.unsplash.com/photo-1594035910387-fea47794261f?w=500&auto=format&fit=crop&q=60' },
//   { id: 3, name: "Minimalist Lounge", category: "HOSPITALITY", desc: "Silence as luxury. This hospitality lounge strips back every non-essential until only the pure geometry and texture remain.", src: 'https://images.unsplash.com/photo-1523293182086-7651a899d37f?w=500&auto=format&fit=crop&q=60' },
//   { id: 4, name: "Startup Atmosphere", category: "CO-WORKING", desc: "Energy bottled in architecture. Exposed structures, bold color zoning, and modular furniture systems that reconfigure instantly.", src: 'https://images.unsplash.com/photo-1592945403244-b3fbafd7f539?w=500&auto=format&fit=crop&q=60' },
//   { id: 5, name: "Industrial Office", category: "CORPORATE", desc: "Raw concrete slabs meet precision polished steel frameworks to build a productive, textured minimalist modern work hub.", src: 'https://images.unsplash.com/photo-1588405748373-122b2321bc31?w=500&auto=format&fit=crop&q=60' },
//   { id: 6, name: "High-Tech Station", category: "CORPORATE", desc: "The command center of a technology firm, built with integrated sensor lighting, seamless wireless arrays, and ergonomic design lines.", src: 'https://images.unsplash.com/photo-1616949755610-8c9bbc08f138?w=500&auto=format&fit=crop&q=60' },
//   { id: 7, name: "Executive Suite", category: "EXECUTIVE", desc: "Panoramic views framed by dark walnut paneling and custom matte black metal architectural features.", src: 'https://images.unsplash.com/photo-1595425970377-c9703cf48b6d?w=500&auto=format&fit=crop&q=60' },
//   { id: 8, name: "Creative Studio", category: "STUDIO", desc: "An open concept floorplan maximizing northern daylight conditions with tall industrial multi-pane windows.", src: 'https://images.unsplash.com/photo-1608571423902-eed4a5ad8108?w=500&auto=format&fit=crop&q=60' },
// ];
...
//     // ScrollTimeline pinning setup matching layout expectations
...
//     // Make sure elements start stacked neatly
//     gsap.set(items, { x: 0, y: 0, rotateZ: 0, scale: 1 });
//     gsap.set(centralTextRef.current, { opacity: 0, scale: 0.8 });

//     // --- STAGE 1: Spread out horizontally into a neat linear row ---
//     tl.to(items, {
//       x: (i) => (i - (totalItems - 1) / 2) * 170,
//       y: 120,
//       rotateZ: 0,
//       scale: 0.9,
//       duration: 1.5,
...
//     // --- STAGE 2: Circular Wheel configuration with visible middle logo ---
//     tl.to(items, {
//       x: (i) => {
//         const angle = (i / totalItems) * Math.PI * 2 - Math.PI / 2;
//         return Math.cos(angle) * 320;
//       },
//       y: (i) => {
//         const angle = (i / totalItems) * Math.PI * 2 - Math.PI / 2;
//         return Math.sin(angle) * 320;
//       },
...
//     // Fade center logo text concurrently during circle formation
...
//     // Fade center logo out right before wheel breaks down
//     tl.to(centralTextRef.current, {
//       opacity: 0,
//       scale: 0.9,
//       duration: 0.8,
...
//     tl.to(items, {
//       x: (i) => (i - (totalItems - 1) / 2) * 190, 
//       y: (i) => {
//         const distanceFromCenter = i - (totalItems - 1) / 2;
//         return -40 + (distanceFromCenter * distanceFromCenter) * 12; 
//       },
//       rotateZ: (i) => (i - (totalItems - 1) / 2) * 6,
//       scale: 1,
//       duration: 2,
//       ease: 'power3.out'
//     }, '+=0.1');
...
//   return (
//     // 1. Overall component responsive height framework context rule
//     <div ref={containerRef} className="min-h-[120%] bg-white text-neutral-900 relative">
      
//       {/* Intro Header Section */}
//       <div className="h-screen flex flex-col justify-center items-center bg-white z-20 relative">
...
//       <div ref={triggerRef} className="h-screen w-full relative overflow-hidden flex items-center justify-center bg-white">
        
//         {/* Dynamic central wheel text content wrapper */}
...
//         {/* 3D Transform Space Viewport Wrapper */}
//         <div className="relative w-full h-full flex items-center justify-center [perspective:1400px] [transform-style:preserve-3d] z-10">
//           {PERFUMES.map((perfume, index) => (
//             <div
//               key={perfume.id}
//               onClick={() => setActiveIdx(index)}
//               className="gallery-item absolute w-44 h-60 cursor-pointer select-none origin-center group [transform-style:preserve-3d]"
//             >
//               {/* Inner card container providing 3D flip functionality */}
//               <div className="relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                
//                 {/* CARD FRONT SIDE (2. Borders entirely removed) */}
//                 <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] overflow-hidden rounded-md shadow-md">
//                   <img 
//                     src={perfume.src} 
//                     alt={perfume.name} 
...
//                 {/* CARD BACK SIDE (3. Activates on hover, showing grey/slate details) */}
//                 <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] bg-white text-neutral-900 flex flex-col justify-center items-center text-center p-4 rounded-md shadow-lg border border-neutral-100">
//                   <span className="text-[10px] tracking-widest uppercase text-neutral-400 mb-1">Project</span>
//                   <h3 className="text-sm font-semibold tracking-wide text-neutral-800 uppercase px-1 line-clamp-2">{perfume.name}</h3>
//                   <p className="text-[11px] text-slate-400 font-medium tracking-wider uppercase mt-3">Tap to open</p>
//                 </div>
...
//         {/* Video Bottom-Center Title Frame Placement */}
...
//       <div className="h-screen bg-neutral-50 flex flex-col justify-center items-center text-neutral-800 z-10 relative">
...
//       {/* 4. MODAL POPUP DISPLAY LAYER */}
//       {activeIdx !== null && (
//         <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
//           <div className="bg-white text-neutral-900 rounded-sm max-w-4xl w-full flex flex-col md:flex-row overflow-hidden shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            
//             <button 
//               onClick={() => setActiveIdx(null)}
//               className="absolute top-4 right-4 text-neutral-400 hover:text-neutral-700 bg-neutral-50 hover:bg-neutral-100 rounded-full p-2 transition-colors z-10"
//             >
//               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
//             </button>

//             {/* LEFT MEDIA MODULE CONTAINER */}
//             <div className="relative w-full md:w-1/2 aspect-[4/3] md:aspect-auto bg-neutral-900 flex items-center justify-center">
//               <img 
...
//                 className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-neutral-800 rounded-full p-2.5 shadow-md hover:scale-105 transition-all"
//               >
...
//                 className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/90 hover:bg-white text-neutral-800 rounded-full p-2.5 shadow-md hover:scale-105 transition-all"
//               >
...
//               <div className="absolute bottom-4 left-4 bg-black/50 text-white text-[10px] px-3 py-1 rounded-sm font-mono tracking-widest">
//                 {String(activeIdx + 1).padStart(2, '0')} — {String(PERFUMES.length).padStart(2, '0')}
//               </div>
//             </div>

//             {/* RIGHT DESCRIPTION CONTENT MODULE */}
...
//               </span>
//               <h2 className="text-3xl font-normal tracking-wide text-neutral-800 mb-4">
//                 {PERFUMES[activeIdx].name}
//               </h2>
//               <div className="w-10 h-[2px] bg-neutral-900 mb-6"></div>
//               <p className="text-xs leading-relaxed text-neutral-500 font-light">
//                 {PERFUMES[activeIdx].desc}
//               </p>
              
//               <div className="mt-8 flex gap-2 flex-wrap">
//                 <span className="text-[9px] uppercase tracking-widest bg-neutral-50 text-neutral-400 px-2.5 py-1.5 rounded-sm border border-neutral-100">Workspace</span>
//                 <span className="text-[9px] uppercase tracking-widest bg-neutral-50 text-neutral-400 px-2.5 py-1.5 rounded-sm border border-neutral-100">Gallery</span>
//                 <span className="text-[9px] uppercase tracking-widest bg-neutral-50 text-neutral-400 px-2.5 py-1.5 rounded-sm border border-neutral-100">Material Study</span>
//               </div>
//             </div>
...
//     // STAGE 2: Circular Wheel configuration (1. Reduced radius to 240 and scale to 0.75 for screen fitness)
//     tl.to(items, {
//       x: (i) => {
//         const angle = (i / totalItems) * Math.PI * 2 - Math.PI / 2;
...
//       {/* 2. MODAL POPUP COMPONENT (Matches the layout structure of your provided image) */}
//       {activeIdx !== null && (
//         <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setActiveIdx(null)}>
//           <div 
//             className="bg-white text-neutral-900 max-w-5xl w-full flex flex-col md:flex-row overflow-hidden shadow-2xl relative min-h-[500px]"
//             onClick={(e) => e.stopPropagation()}
//           >
            
//             {/* 3. CLOSE BUTTON (With requested rotation and shrink hover scaling) */}
//             <button 
//               onClick={() => setActiveIdx(null)}
//               className="absolute top-6 right-6 text-neutral-400 hover:text-neutral-700 bg-neutral-100/80 rounded-full p-2.5 transition-all duration-500 transform hover:rotate-180 hover:scale-90 z-20"
//             >
//               <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
//             </button>
...
//                 className="absolute left-4 top-1/2 -translate-y-1/2 bg-white text-neutral-800 rounded-full p-3 shadow-md hover:scale-105 transition-all z-10"
//               >
//                 <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M15 19l-7-7 7-7" /></svg>
//               </button>
//               <button 
//                 onClick={(e) => navigateModal('next', e)}
//                 className="absolute right-4 top-1/2 -translate-y-1/2 bg-white text-neutral-800 rounded-full p-3 shadow-md hover:scale-105 transition-all z-10"
//               >
//                 <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 5l7 7-7 7" /></svg>
//               </button>

//               {/* Dynamic Image Counter Display Box */}
//               <div className="absolute bottom-6 left-6 bg-black/50 text-white text-[11px] px-3 py-1.5 rounded-sm font-mono tracking-widest">
//                 {String(activeIdx + 1).padStart(2, '0')} — {String(PERFUMES.length).padStart(2, '0')}
//               </div>
//             </div>
...
//                 <span className="text-[10px] uppercase tracking-widest bg-neutral-50 text-neutral-400 px-4 py-2 rounded-sm border border-neutral-100 font-medium">Workspace</span>
//                 <span className="text-[10px] uppercase tracking-widest bg-neutral-50 text-neutral-400 px-4 py-2 rounded-sm border border-neutral-100 font-medium">Gallery</span>
//                 <span className="text-[10px] uppercase tracking-widest bg-neutral-50 text-neutral-400 px-4 py-2 rounded-sm border border-neutral-100 font-medium">Material Study</span>
//               </div>
//             </div>
...
export default function ScrollGallery() {
  ...
  useGSAP(() => {
    ...
    // STAGE 2: Circular Wheel configuration
    tl.to(items, {
      x: (i) => {
        const angle = (i / totalItems) * Math.PI * 2 - Math.PI / 2;
        return Math.cos(angle) * 240;
        ...
        <div className="relative w-full h-full flex items-center justify-center [perspective:1400px] [transform-style:preserve-3d] z-10">
          {PERFUMES.map((perfume, index) => (
            <div
              key={perfume.id}
              onClick={() => setActiveIdx(index)}
              className="gallery-item absolute w-44 h-60 cursor-pointer select-none origin-center group [transform-style:preserve-3d] hover:z-30 will-change-transform"
            >
              <div className="relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                
                {/* CARD FRONT SIDE */}
                <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] overflow-hidden rounded-none shadow-md">
                  <img 
                    src={perfume.src} 
                    alt={perfume.name} 
                    className="w-full h-full object-cover pointer-events-none" 
                ...
                {/* CARD BACK SIDE */}
                <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] bg-white text-neutral-900 flex flex-col justify-center items-center text-center p-4 rounded-none shadow-lg border border-neutral-100 z-10">
                  <span className="text-[10px] tracking-widest uppercase text-neutral-400 mb-1">Project</span>
                  <h3 className="text-sm font-semibold tracking-wide text-neutral-800 uppercase px-1 line-clamp-2">{perfume.name}</h3>
                  <p className="text-[11px] text-slate-400 font-medium tracking-wider uppercase mt-3">Tap to open</p>
                </div>
